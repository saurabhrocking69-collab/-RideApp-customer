...
export function IntercityScreen() {
  ...
  const handleBook = async () => {
    if (loading) return;
    if (leaveMode === 'later' && !scheduledAt) { Alert.alert('Pick departure time', 'Choose when your trip starts.'); return; }
    if (tripKind === 'round' && !returnAt) { Alert.alert('Pick return time', 'Choose when your round trip ends.'); return; }
    if (tripKind === 'round' && returnAt) {
      ...
      if (new Date(returnAt.iso).getTime() <= leaveMs) {
        Alert.alert('Invalid return time', 'Return must be after departure.'); return;
      }
    }
    await bookIntercity({
        ...
        >
          {loading ? (
            ...
            <Text style={{ fontSize: 16, fontWeight: '900', color: '#fff' }}>
              {selOpt ? `Book ${selOpt.label} — ₹${selOpt.fare}` : 'Book Intercity'}
            </Text>
          )}
        </TouchableOpacity>
      </View>
