...
export function DeleteAccountSheet({
  visible, onClose, phone, role = 'customer', onDeleted,
}: {
  visible: boolean;
  onClose: () => void;
  phone: string;
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.45)', justifyContent: 'flex-end' }}>
        <View style={{ backgroundColor: C.bgCard, borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: '88%' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', padding: 18, borderBottomWidth: 1, borderBottomColor: C.glassBorder }}>
            <Text style={{ flex: 1, fontSize: 17, fontWeight: '900', color: C.text }}>Delete account</Text>
          ...
          <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 28 }}>
            {loading ? (
              ...
              // ── Already requested ──
              <>
                <View style={{ backgroundColor: 'rgba(245,158,11,0.12)', borderRadius: R.xs, padding: 14, borderWidth: 1, borderColor: 'rgba(245,158,11,0.35)' }}>
                ...
                <Text style={{ color: C.textMuted, fontSize: 12.5, lineHeight: 19, marginTop: 16 }}>
                ...
                <TouchableOpacity onPress={cancelRequest} disabled={busy}
                  style={{ marginTop: 16, backgroundColor: C.green, borderRadius: R.xs, paddingVertical: 15, alignItems: 'center', opacity: busy ? 0.6 : 1 }}>
              ...
              <>
                ...
                {blockers.map((b: string, i: number) => (
                  <View key={i} style={{ flexDirection: 'row', gap: 9, marginBottom: 9 }}>
                    ...
                    <Text style={{ flex: 1, color: C.textMuted, fontSize: 13, lineHeight: 19 }}>{b}</Text>
                  </View>
                ))}
                <TouchableOpacity onPress={load}
                  style={{ marginTop: 14, borderWidth: 1.5, borderColor: C.glassBorder, borderRadius: R.xs, paddingVertical: 13, alignItems: 'center' }}>
                  <Text style={{ color: C.text, fontWeight: '800', fontSize: 14 }}>Check again</Text>
                </TouchableOpacity>
              </>
            ) : (
              ...
              <>
                ...
                <Text style={{ color: C.text, fontSize: 12.5, fontWeight: '900', marginTop: 18, marginBottom: 8, letterSpacing: 0.6 }}>
                  WHAT GETS DELETED
                </Text>
                {['Your name, phone number and profile',
                  ...
                  <View key={i} style={{ flexDirection: 'row', gap: 9, marginBottom: 7 }}>
                    ...
                    <Text style={{ flex: 1, color: C.textMuted, fontSize: 12.5, lineHeight: 18 }}>{t}</Text>
                  </View>
                ))}

                <Text style={{ color: C.text, fontSize: 12.5, fontWeight: '900', marginTop: 14, marginBottom: 8, letterSpacing: 0.6 }}>
                  WHAT WE HAVE TO KEEP
                </Text>
                <View style={{ flexDirection: 'row', gap: 9 }}>
                  <Ionicons name="receipt-outline" size={15} color={C.textDim} style={{ marginTop: 2 }} />
                ...
                {notes.map((n: string, i: number) => (
                  <View key={i} style={{ flexDirection: 'row', gap: 9, marginTop: 12, backgroundColor: 'rgba(245,158,11,0.10)', borderRadius: R.xs, padding: 11, borderWidth: 1, borderColor: 'rgba(245,158,11,0.30)' }}>
                ...
                {!confirming ? (
                  <TouchableOpacity onPress={() => setConfirming(true)}
                    style={{ marginTop: 20, borderWidth: 1.5, borderColor: C.red, borderRadius: R.xs, paddingVertical: 14, alignItems: 'center' }}>
                    <Text style={{ color: C.red, fontWeight: '900', fontSize: 14.5 }}>Continue</Text>
                  </TouchableOpacity>
                ) : (
                  <>
                    <Text style={{ color: C.text, fontSize: 12.5, fontWeight: '800', marginTop: 20, marginBottom: 7 }}>
                    ...
                    <TextInput
                      ...
                      style={{ borderWidth: 1.5, borderColor: C.glassBorder, borderRadius: R.xs, padding: 12, minHeight: 70, color: C.text, fontSize: 13, textAlignVertical: 'top' }}
                    />
                    <TouchableOpacity onPress={submit} disabled={busy}
                      style={{ marginTop: 14, backgroundColor: C.red, borderRadius: R.xs, paddingVertical: 15, alignItems: 'center', opacity: busy ? 0.6 : 1 }}>
                      <Text style={{ color: '#fff', fontWeight: '900', fontSize: 15 }}>
                    ...
                    <TouchableOpacity onPress={() => setConfirming(false)} style={{ paddingVertical: 12, alignItems: 'center' }}>
                      <Text style={{ color: C.textMuted, fontWeight: '700', fontSize: 13.5 }}>Not now</Text>
                    </TouchableOpacity>
                  </>
                )}
              </>
